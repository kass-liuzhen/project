...
async def get_course_list(request):
    with db_block() as db:
        db.execute("""
        SELECT sn AS cou_sn, no AS cou_no, name AS cou_name, place, atime FROM course
        """)
...
async def new_course(request):
    ...
    if not course.get('atime'):
        course['atime'] = datetime.date(1900, 1, 1)

    with db_block() as db:
        db.execute("""
        INSERT INTO course (no, name, place, atime)
        VALUES(%(cou_no)s, %(cou_name)s, %(place)s, %(atime)s) RETURNING sn;
        """, course)
        record = db.fetch_first()
...
async def update_course(request):
    ...
    with db_block() as db:
        db.execute("""
        UPDATE course SET
            no=%(cou_no)s, name=%(cou_name)s, place=%(place)s, atime=%(atime)s
        WHERE sn=%(cou_sn)s;
        """, course)
